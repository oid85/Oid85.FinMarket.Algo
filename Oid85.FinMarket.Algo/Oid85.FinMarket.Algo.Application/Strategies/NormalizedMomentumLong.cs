...
{
    public class NormalizedMomentumLong(
        IDataService dataService)
        : Strategy
    {
        public override string StrategyName { get; set; } = nameof(NormalizedMomentumLong);

        public override string StrategyDescription { get; set; } = "Momentum. Среднеквадратичное отклонение. Только лонг";
        ...
        public override void Execute()
        {
            ...
            for (int i = StabilizationPeriod; i < Candles.Count - 1; i++)
            {
                ...
                if (isBalancing)
                {
                    var topTickers = dataService.GetNormalizedMomentumTopTickers(CandleData, Candles[i].Date, period, percent);
